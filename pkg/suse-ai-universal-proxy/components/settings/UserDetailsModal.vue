        ...
        <form v-if="(!user || isEditing) && formData" @submit.prevent="handleSave" class="user-form">
         ...
         <div v-else class="user-details">
          ...
          <div v-if="!props.isExternal && userPermissions" class="detail-section">
           ...
           <div v-if="isExternal" class="detail-section">
             <h3>Groups</h3>
             <div v-if="userGroups.length > 0" class="groups-list">
               <div
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const confirmPassword = ref('');
     ...
     watch(() => props.user, (newUser) => {
       if (newUser) {
         ...
         // For editing form initialization:
         if (props.isExternal) {
           if ((newUser as ExternalUser).groups) {
             selectedGroups.value = [...(newUser as ExternalUser).groups];
           } else {
             // Fallback: try to derive from availableGroups if already loaded
             const derived = availableGroups.value
               .filter(g => g.members && g.members.includes(newUser.id))
               .map(g => g.id);
             if (derived.length > 0) {
               selectedGroups.value = derived;
             }
           }
         }
    ...
    // Derived user groups computed property
    const userGroups = computed(() => {
      if (!props.isExternal || !props.user) return [];
      
      // Filter groups where the current user ID is in the members list
      return availableGroups.value.filter(group => {
        return group.members && group.members.includes(props.user?.id);
      });
    });

    // Watch for show prop to fetch groups when modal opens
