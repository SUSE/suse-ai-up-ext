  ...
  setup(props) {
    ...
    const loadAppInfo = async () => {
      try {
        ...
        const app = apps.find(a => a.slug_name === props.slug);
        if (app) {
          appInfo.value = app;
        } else {
          error.value = `App "${props.slug}" not found`;
        }
      } catch (err: any) {
    ...
    const loadAppInstances = async () => {
      try {
        const allClusters = await getClusters(store);
    ...
    const silentRefresh = async () => {
      // Skip refresh if paused or if any delete operations are in progress
      if (autoRefreshPaused.value || deletingInstances.value.size > 0) {
        return;
      }
