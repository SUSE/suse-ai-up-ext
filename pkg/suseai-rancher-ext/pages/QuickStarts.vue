   ...
   <main class="main-layout">
    <div class="outlet">
      ...
      <header class="fixed-header">
        <!-- Page Title -->
        <div class="title">
          <h1 class="m-0" id="page-title">Quick Starts</h1>
        </div>
        ...
        <div class="actions-container" role="toolbar" aria-label="Application filters and actions">
          <div class="search-box">
            ...
            <input
              ...
              :aria-describedby="search ? 'search-results-count' : undefined"
            />
          </div>
      ...
      <div v-if="repoLoading && !loading" class="repo-loading" aria-live="polite">
        <div class="loading-banner">
          ...
          <span>Loading repository applications...</span>
        </div>
      </div>
      ...
      <div class="main-content">
        ...
        <div v-if="!loading && !filteredApps.length && !error" class="empty-state-content">
          ...
          <p class="text-muted">Try adjusting your search or repository filter.</p>
        </div>
      </div>
    </div>
  </main>
...
import { fetchSuseAiApps, fetchClusterRepositories, fetchAppsFromRepository } from '../services/app-collection';
...
export default defineComponent({
  ...
  setup() {
    ...
    const repositoryOptions = computed(() => {
      const options = [];
      const suseaiBundleRepo = repositories.value.find(repo => repo.name === 'suseai-bundles');
      if (suseaiBundleRepo) {
        options.push({
          label: suseaiBundleRepo.displayName || suseaiBundleRepo.name,
          value: suseaiBundleRepo.name
        });
      }
      return options;
    });
    ...
    const refresh = async () => {
      ...
      try {
        ...
        // Set selectedRepo to suseai-bundles if available
        const suseaiBundleRepo = repositories.value.find(repo => repo.name === 'suseai-bundles');
        if (suseaiBundleRepo) {
          selectedRepo.value = suseaiBundleRepo.name;
          await loadRepositoryApps(suseaiBundleRepo.name);
        }
      } catch (err) {
    ...
    const loadRepositories = async () => {
      try {
        ...
        // Add mock suseai-bundles repository
        const mockRepo = {
          name: 'suseai-bundles',
          displayName: 'SUSE AI Bundles',
          type: 'helm',
          url: 'https://example.com/suseai-bundles'
        };
        repositories.value = [...repos, mockRepo];
        console.log('[SUSE-AI] Found repositories:', repositories.value.map(r => r.name));
      } catch (err) {
    ...
    const loadRepositoryApps = async (repoName: string) => {
      try {
            ...
            const repoApps: any[] = [
              {
                ...
                description: 'A comprehensive bundle for AI assistants including models and tools.',
                packaging_format: 'HELM_CHART' as any,
                logo_url: '/img/generic-app.svg'
              },
              {
                name: 'Data Processing Bundle',
                slug_name: 'data-processing-bundle',
                description: 'Bundle for data processing and analytics with AI integration.',
                packaging_format: 'CONTAINER' as any,
                logo_url: '/img/generic-app.svg'
              },
              {
                name: 'Security AI Bundle',
                slug_name: 'security-ai-bundle',
                description: 'AI-powered security tools and monitoring bundle.',
                packaging_format: 'HELM_CHART' as any,
                logo_url: '/img/generic-app.svg'
              }
            ];
            allRepositoryApps.value[repoName] = repoApps;
          } else {
            const repoApps = await fetchAppsFromRepository(store, repoName);
            allRepositoryApps.value[repoName] = repoApps;
          }
          console.log('[SUSE-AI] Loaded apps from repository:', { repo: repoName, count: allRepositoryApps.value[repoName].length });
        }
      } catch (err) {
        console.error('Failed to load repository apps:', err);
      }
    };
...
.app-tile {
  ...
  .tile-content {
    ...
    .tile-description {
      ...
      font-size: 14px;
      flex: 1;
      min-height: 84px; /* 4 lines * 1.5 line-height * 14px font-size */
      max-height: 84px;
